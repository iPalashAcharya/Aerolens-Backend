...
class DepartmentValidator {
    static validateCreate(req, res, next) {
        const { error } = departmentSchemas.create.validate(req.body, { abortEarly: false }); //abortEearly purpose defined below
        if (error) {
    ...
    static validateUpdate(req, res, next) { //validates both request body and route params, Aggregates errors from both validations into a single details array.
        const { error: bodyError } = departmentSchemas.update.validate(req.body, { abortEarly: false }); //abortEarly false gathers all the validation errors instead of stopping at the first one
